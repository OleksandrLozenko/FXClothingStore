...
public class AdminpaneController implements Initializable {
    ...
    public void setCbUsers() {
       cbUsers.setItems(FXCollections.observableArrayList(getEntityManager()
               .createQuery("SELECT u FROM User u WHERE u.login != :.%аdmin")
               .setParameter("аdmin", "аdmin")
               .getResultList()));
    ...
    public void setCbRoles() {
        cbRoles.setItems(FXCollections.observableArrayList(jptv22clothingstore.JPTV22СlothingStore.roles.values()));
        //Настройка механизма отображения в ComboBox
        cbRoles.setCellFactory(param -> new ListCell<jptv22clothingstore.JPTV22СlothingStore.roles>(){
            @Override
            protected void updateItem(jptv22clothingstore.JPTV22СlothingStore.roles item,boolean empty){
        ...
        cbRoles.setButtonCell(new ListCell<jptv22clothingstore.JPTV22СlothingStore.roles>() {
            @Override
            protected void updateItem(jptv22clothingstore.JPTV22СlothingStore.roles item, boolean empty) {
                super.updateItem(item, empty);
        ...
        cbRoles.setOnAction(event->{
            this.selectedRole = (jptv22clothingstore.JPTV22СlothingStore.roles) cbRoles.getValue();
